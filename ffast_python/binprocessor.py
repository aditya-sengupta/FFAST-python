...
class BinProcessor:
    ...
    def get_omega(self, i):
        ...
        for j in range(self.delays_per_bunch_nb - 1):
            self.angles[j] = np.angle(np.conj(self.observation_matrix[self.bin_absolute_index][i * self.delays_per_bunch_nb + self.delay_index + 1]))
    ...
    def estimate_bin_signal(self):
        ...
        for i in range(len(delays)):
            ...
            amplitude += np.conj(self.dirvector[i]) * self.observation_matrix[self.bin_absolute_index][i]

        self.amplitude = amplitude / self.delays_nb
        noise = 0
        for i in range(self.delays_nb):
            self.signal_vector[i] = self.amplitude * self.dirvector[i]
            noise += norm_squared(self.observation_matrix[self.bin_absolute_index][i] - self.signal_vector[i])
    ...
    def adjust_to(self, new_bin_abs_index, new_bin_rel_index, new_stage):
        ...
        self.bin_size = config.bin_size[stage]
    ...
    def compute_thresholds(self):
        self.energy_bins = np.zeros(config.bins_sum)
        if config.is_noisy or config.quantize or config.apply_window_var:
            for stage in range(config.get_bins_nb):
                for i in range(len(self.bins[stage])):
                    self.energy_bins[i] = norm_squared(self.observation_matrix[config.bin_offset[stage] + i])
        ...
        if not (config.is_noisy or config.quantize or config.apply_window_var):
            self.minimum_energy = 1e-8
        elif (config.is_noisy or config.quantize and not config.apply_window_var):
            self.minimum_energy = min(0.1 * noise_estimation * pow(10, config.SNR_dB / 10), 1000 * noise_estimation)
        
        if config.apply_window_var:
            self.minimum_energy = 0.1 * config.min_fourier_magnitude
        ...
        if config.apply_window_var or config.is_noisy or config.quantize:
    ...
    def compute_weights(self):
        ...
        self.weights = base_weight * np.fromfunction(lambda i: (i + 1) * (self.delays_per_bunch_nb  - (i + 1)))